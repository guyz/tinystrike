...
// Tells the post chain that a material shader will read `owSsaoTex`, so it is
// worth building the AO targets and running the passes. Module scope on purpose:
// main.js imports materials/index.js (and therefore this file) before it
// constructs PostChain, so the flag is set by the time the chain looks at it.
registerSsaoConsumer();
...
export function extendMaterial(material, p, shared) {
  ...
  // The three SSAO uniforms are module singletons shared by every material, so
  // the post chain writes `.value` once a frame and every shader in the scene
