 ...
 *
 * AXIS MAPPING, and it is the single most-repeated modelling bug in this kit.
 * The outline lives in XY. The depth is ALWAYS Z and it is ALWAYS depth-centred
 * (the translate below re-centres what ExtrudeGeometry builds from z=0 forward).
 * So an outline authored as a SIDE view — first coordinate fore/aft, second
 * up/down — is standing across the weapon like a cattle guard until it is
 * turned, and one `add()` cannot always do it, because `Assembly.add` composes
 * its Euler in 'XYZ' order (see the note there).
 *
 * THE RULE: bake the roll into the geometry with `g.rotateY(+/-PI/2)` right
 * after the extrude and say which way the outline's axes landed, then pass only
 * a position (and at most one further rotation) to `add()`. Six measured
 * defects — the AR ejection port 8.7 mm proud of a 19 mm receiver, the pistol
 * port 36 mm across a 26 mm slide, the magazine feed lips 60-100 mm wide, the
 * M-LOK plates lying flat, the trigger blade across the guard, the AK stock —
 * were all this one mistake. The three places that got it right (triggerPart,
 * addLowerReceiver's guard, ak.js's stock) all bake it first, and they say so.
 */
 ...
 * note on `Assembly.add` calls for.
 *
 * Derivation, for the 'XYZ' order three actually composes (M = Rx*Ry*Rz):
 *   Rz leaves +Z alone, so M*(0,0,1) = Rx(a)*Ry(b)*(0,0,1)
 *                                    = (sin b, -sin a cos b, cos a cos b)
 * which inverts to b = asin(dx), a = atan2(-dy, dz) for any unit dir. `roll`
 * then falls through to rz, where it is free: it spins the part about its own
 * axis, which is what rz is genuinely good for.
 *
 * Straight up (dz = 0, dy = 1) is a legal input and gives rx = -PI/2; the only
 * singularity is |dx| = 1 (dead sideways), where rx becomes arbitrary — which
 * is correct, because a rod pointing along X has no distinguishable pitch.
 */
export function alignZ(dx, dy, dz, roll = 0) {
  const l = Math.hypot(dx, dy, dz) || 1;
  const x = dx / l;
  return {
    rx: Math.atan2(-dy / l, dz / l),
    ry: Math.asin(Math.max(-1, Math.min(1, x))),
    rz: roll,
  };
}
...
export class Assembly {
   ...
   *
   * EULER ORDER: 'XYZ', which in three means the matrix is Rx*Ry*Rz — so **rz
   * is applied FIRST**, then ry, then rx.
   *
   * The consequence is not obvious and it cost four floating parts. Every axial
   * primitive here (`rodZ`, `tubeZ`, `latheZ`, `screw`) has its axis on +Z, and
   * +Z is INVARIANT under rz. So `{ ry, rz }` on a rod silently discards the rz
   * as far as the axis is concerned: the rod ends up on whatever axis ry alone
   * gives it, and any part hand-placed at the end of the intended cant misses.
   * MEASURED, before the fix: the MP5's cocking knob sat 5.2 mm clear of its
   * stalk and the AWP's bolt knob 4.4 mm clear of its own, in both cases because
   * the 0.42-0.72 rad cant never happened.
   *
   * `rz` is still useful on an axial primitive — as ROLL, e.g. clocking a
   * knurl or an M-LOK plate round the barrel. It just cannot aim one.
   *
   * To AIM an axial primitive, use `alignZ(dx, dy, dz, roll)` below, which
   * solves this exact Euler order for the direction you actually want.
   */
  add(geo, mat, t = null) {
    const g = geo.clone();
    if (t) {
