...
export function buildRifle(o = {}) {
  ...
  const hgR = 0.0235;
  const zBreech = -0.1;
  const zBarrelEnd = o.barrelEnd ?? -0.44;
  const opticY = bore + (o.opticRise ?? 0.067);
  const opticZ = o.opticZ ?? -0.022;

  /**
   * Variant controls. Tiny Strike derives three long guns from this receiver —
   * a flat-top carbine with a red dot, an east-bloc pattern rifle on irons with
   * timber furniture, and a bolt-action with a magnified scope — because they
   * share a receiver architecture and differ in furniture, barrel and sights,
   * which is exactly what these switches are.
   */
  const furniture = o.furniture ?? 'polymer';   // handguard panels, grip, stock
  const muzzleKind = o.muzzle ?? 'brake';
  const opticOn = o.optic !== false;
  const opticR = o.opticRadius ?? 0.0155;
  const opticLen = o.opticLen ?? 0.052;
  const gasOn = o.gasBlock !== false;
  const boltHandle = o.boltHandle === true;
  const handguardR = o.handguardRadius ?? hgR;
  ...
  const muzzle = addMuzzleDevice(body, 'steel_soot', 'cavity', muzzleKind, zBarrelEnd, 0.0077, bore);
  ...
  /**
   * A bolt-action's handle: a knob on a stalk, canted down and back on the
   * right of the receiver. It is the single silhouette cue that separates a
   * bolt gun from a self-loader at any distance, so it is geometry, not paint.
   */
  if (boltHandle) {
    const stalk = rodZ(0.0042, 0.0042, 0.062, 12);
    body.add(stalk, 'steel', { x: rUpper + 0.012, y: bore - 0.004, z: 0.012, ry: Math.PI / 2, rz: 0.22 });
    stalk.dispose();
    const knob = dome(0.0105, 14, 1);
    body.add(knob, 'steel_bright', { x: rUpper + 0.052, y: bore - 0.018, z: 0.012 });
    body.add(knob, 'steel_bright', { x: rUpper + 0.052, y: bore - 0.018, z: 0.012, rx: Math.PI });
    knob.dispose();
    // Bolt shroud at the rear of the receiver.
    const shroud = latheZ([[0, 0], [0, 0.0138], [0.014, 0.0138], [0.02, 0.0104], [0.02, 0]], 16);
    body.add(shroud, 'steel_black', { y: bore, z: zUpperRear + 0.004 });
    shroud.dispose();
  }
   ...
   */
  addFrontSight(body, 'polymer', 'alu', 0, railTop, o.frontSightZ ?? -0.358, !opticOn);
  /**
   ...
   */
  addRearSight(body, 'polymer', 'alu', 0, railTop, o.rearSightZ ?? -0.112, !opticOn);

  // ---- moving parts --------------------------------------------------------
  /**
   * 30-ROUND STANAG: 191 mm OVERALL, 14 mm OF FEED CURVE.
   *
   * `len` is NOT the magazine's height. buildMagazine puts the feed lips 8.0 mm
   * ABOVE the arc's origin and hangs the floorplate, finger ledge and rubber
   * base pad BELOW `-len`, all three rotated onto the arc's end tangent — so the
   * box measures len + 8 + ~17 mm. At len = 212 mm that came out at
   *   MEASURED 240.9 mm against a real STANAG's 191 mm  (+26%)
   * i.e. the game shipped a 7.62-sized box on a 5.56 carbine, which is exactly
   * how it read: too deep under the magwell and curved like an AK's.
   *
   * Solved by bisection (not eyeballed) for a 191.0 mm box: len = 165.1 mm,
   * which leaves 165 mm of body + 8 mm of lips + 17 mm of floorplate stack.
   * MEASURED AFTER: 191.0 mm (0.0%).
   *
   * The curve came down with it. 30 mm of sagitta over 212 mm is 14% — that is
   * an AK's arc, and 7.62x39 needs it because the case is steeply tapered. A
   * GI/PMAG 30-rounder's floorplate stands about 14 mm forward of the feed-lip
   * axis (7.3% of 191 mm), because 5.56 NATO's case body tapers only ~0.5 mm
   * over 35 mm. Dropping 30 -> 14 mm also takes the magazine's front-to-back
   * envelope from 100.0 mm to 84.0 mm, since the old arc was swinging the toe
   * 30 mm out past the magwell's front wall.
   */
  ...
  const mag = buildMagazine(magazine, null, {
    ...
    d: o.magD ?? 0.0655,
    len: o.magLen ?? 0.1651,
    curve: o.magCurve ?? 0.014,
    segs: 8,
    witness: 4,
    poly: o.magMat ?? furniture,
  });
  ...
  return {
    ...
    nodes: {
      ...
      sight: [0, opticOn ? opticY : railTop + 0.019, optic.lensZ],
      sightAxis: [0, 0, -1],
      ...
      /**
       * SEATED CONCENTRIC WITH ITS OWN MAGWELL, which it was not.
       *
       * Both the well and the magazine are tilted `magTilt` forward, so they are
       * parallel lines — but the well is placed at z = magZ at ITS OWN centre
       * height (magTop+magBottom)/2 = 28.5 mm, while the magazine hangs from
       * y = 61 mm. Two parallel raked lines through the same z at different
       * heights are 2.6 mm apart: sin(0.08) * (61 - 28.5) = 2.60 mm, with the
       * magazine forward of the well.
       *
       * MEASURED at the well's mid-height, mag body 65.5 mm deep in a mouth
       * flange 72.2 mm deep: front faces 0.55 mm PROUD of the well's outside,
       * rear face 4.65 mm inside it — the flange read as a one-sided ledge behind
       * the magazine and the mag's front corner broke the well's front line.
       * Adding the 2.60 mm back puts it at 2.0 mm inset on BOTH faces, which is
       * what a flared magwell mouth looks like.
       */
      magSeat: { pos: [0, 0.061, magZ + Math.sin(magTilt) * (0.061 - 0.0285)], rot: [magTilt, 0, 0] },
      magDrop: [0, -0.4, 0.02],
